...
Mailer.send = function (options, cb) {
  ...
  if (connector.mandrill) {
    ...
    else if (_.isArray(options.to) && _.isString(options.to[0])) {
      // array of email strings
      mandrilMessage.message.to = [];
    ...
    if (typeof options.template === 'object') {
      ...
      mandrilMessage.message.global_merge_vars = options.template.global_merge_vars || [];

      if (options.template.merge_vars) {
        var merge_vars = options.template.merge_vars;

        mandrilMessage.message.merge_vars = [];
        if (mandrilMessage.message.to.length < 2) {
          mandrilMessage.message.merge_vars = [{
            rcpt: mandrilMessage.message.to[0].email,
            vars: merge_vars
          }];
        } else {
          _.each(mandrilMessage.message.to, function(email, index) {
            mandrilMessage.message.merge_vars[index] = {
              rcpt: email.email,
              vars: []
            };
            _.forOwn(merge_vars, function(content, name) {
              mandrilMessage.message.merge_vars[index].vars.push( {
                name: name,
                content: content(email.email, index)
              });
            })
          });
        }
    ...
    console.warn('Warning: no connection with Mandrill');
