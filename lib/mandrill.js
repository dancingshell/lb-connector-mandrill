...
Mailer.send = function (options, cb) {
  ...
  if(connector.mandrill) {
    ...
    if(options.template) {
      ...
      connector.mandrill.messages.sendTemplate(mandrilMessage, function(result) { fn(null, result); }, function(err) { fn(err, null); });
    }
    else {
      _.extend(mandrilMessage.message, _.extend(options, settings.defaults))
      _.extend(mandrilMessage, {
      ...
      connector.mandrill.messages.send(mandrilMessage, function(result) { fn(null, result); }, function(err) { fn(err, null); });
    }
  } else {
